...
public class PdfReportService {
    ...
    private static Document createBaseDocument(Context context, String fileName) throws Exception {
        File folder = new File(context.getExternalFilesDir(Environment.DIRECTORY_DOCUMENTS), "SanataniReports");
        if (!folder.exists()) folder.mkdirs();
        File file = new File(folder, fileName + "_" + System.currentTimeMillis() + ".pdf");
    ...
    public static void generateMemberDirectory(Context context, String communityName, List<Member> members) {
        try {
            Document document = createBaseDocument(context, "Member_Directory");
            ...
            Toast.makeText(context, "Directory PDF Saved to Documents!", Toast.LENGTH_LONG).show();
    ...
    public static void generateMemberProfile(Context context, String communityName, Member m) {
        try {
            Document document = createBaseDocument(context, "Devotee_Profile_" + m.id);
            ...
            Toast.makeText(context, "Profile PDF Saved to Documents!", Toast.LENGTH_LONG).show();
        } catch (Exception e) { Toast.makeText(context, "Error generating profile PDF", Toast.LENGTH_SHORT).show(); }
    }
    ...
    public static void generateLoginCredentialsPdf(Context context, String communityName, String name, String memberId, String pin, String generatedBy) {
        try {
            Document document = createBaseDocument(context, "Secure_Credentials_" + memberId);
    ...
    public static void generateFinancialReport(Context context, String communityName, List<String> dates, List<String> names, List<Float> amounts, List<String> notes, float totalCollected, String title) {
        try {
            Document document = createBaseDocument(context, "Master_Chanda_Ledger");
            ...
            addSanataniFooter(document); document.close();
            Toast.makeText(context, "Financial Ledger PDF Saved!", Toast.LENGTH_LONG).show();
        } catch (Exception e) {}
    }

    public static void generateDonorStatement(Context context, String communityName, TransactionActivity.GroupedDonation gd) {
        try {
            Document document = createBaseDocument(context, "Chanda_Statement_" + gd.displayName.replaceAll("[^a-zA-Z0-9]", ""));
    ...
    public static void generateExpenseReport(Context context, String communityName, List<ExpenseActivity.Expense> expenses, float totalSpent, String title) {
        try {
            Document document = createBaseDocument(context, "Master_Utsav_Expense");
            addSanataniHeader(document, communityName, title);
            ...
            addSanataniFooter(document); document.close();
            Toast.makeText(context, "Expense Report PDF Saved!", Toast.LENGTH_LONG).show();
        } catch (Exception e) {}
    }

    public static void generateUtsavStatement(Context context, String communityName, ExpenseActivity.GroupedExpense ge) {
        try {
            Document document = createBaseDocument(context, "Utsav_Statement_" + ge.eventDisplayName.replaceAll("[^a-zA-Z0-9]", ""));
            ...
            addSanataniFooter(document); document.close();
            Toast.makeText(context, "Utsav Statement PDF Saved!", Toast.LENGTH_LONG).show();
        } catch (Exception e) {}
    }
    ...
    public static void generatePollReport(Context context, String communityName, Poll poll, boolean includeVoterNames) {
        try {
            ...
            if (poll.adminComment != null && !poll.adminComment.isEmpty()) {
                document.add(new Paragraph("Admin Note: " + poll.adminComment).setItalic().setFontColor(ColorConstants.DARK_GRAY).setMarginBottom(10));
            }
            ...
            addSanataniFooter(document); document.close();
            Toast.makeText(context, "Poll Insight PDF Saved!", Toast.LENGTH_LONG).show();
        } catch (Exception e) {}
    }
    ...
    public static void generateMultiplePollsReport(Context context, String communityName, List<Poll> polls, String title, boolean includeVoterNames) {
        try {
            ...
            for (Poll poll : polls) {
                ...
                if (includeVoterNames && poll.votes != null) {
                    document.add(new Paragraph("Voter IDs: " + poll.votes.keySet().toString()).setFontSize(9).setFontColor(ColorConstants.GRAY));
                }
                document.add(new Paragraph("--------------------------------------------------").setFontColor(ColorConstants.LIGHT_GRAY));
            }
            addSanataniFooter(document); document.close();
            Toast.makeText(context, "Master Polls PDF Saved!", Toast.LENGTH_LONG).show();
        } catch (Exception e) {}
    }
    ...
    public static void generateEventItinerary(Context context, String communityName, EventActivity.Event event, String generatedBy) {
        try {
            Document document = createBaseDocument(context, "Mandir_Event_" + event.title.replaceAll("[^a-zA-Z0-9]", ""));
            ...
            addSanataniFooter(document); document.close();
            Toast.makeText(context, "Event Itinerary PDF Saved!", Toast.LENGTH_LONG).show();
        } catch (Exception e) {}
    }
    ...
    public static void generateMasterEventReport(Context context, String communityName, List<EventActivity.Event> events, String title) {
        try {
            ...
            addSanataniFooter(document); document.close();
            Toast.makeText(context, "Master Event Calendar PDF Saved!", Toast.LENGTH_LONG).show();
        } catch (Exception e) {}
    }
